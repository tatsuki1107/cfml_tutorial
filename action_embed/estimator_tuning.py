...
from estimator import MarginalizedIPS
...
class SLOPE(BaseOffPolicyEstimatorWithTune):
    ...
    def __post_init__(self) -> None:
        ...
        if not self.tuning_method in {"exact_scalar", "exact_combination", "greedy_combination"}:
            raise ValueError
    ...
    def estimate_policy_value_with_tune(self, bandit_feedback: dict, action_dist: np.ndarray) -> np.float64:
        ...
        if self.tuning_method == "greedy_combination":
            estimated_policy_value = self._tune_combination_with_greedy_pruning(bandit_feedback=bandit_feedback, action_dist=action_dist)
    ...
    def _tune_combination_with_greedy_pruning(self, bandit_feedback: dict, action_dist: np.ndarray):
